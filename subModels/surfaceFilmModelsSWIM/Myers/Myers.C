...
void Myers::updateSubmodels()
{
	...
	Info<<"magSf() "<<magSf()<<nl;
	Info<<"time().deltaT() "<<time().deltaT()<<nl;
	Info<<"!!!rMagSfDt " << rMagSfDt << nl;
*/
    turbulence_->correct();
}
...
void Myers::solveEnergy()
{
    ...
    //volScalarField TIce = 
	Ts_ = Tw_ + (q_0r + q_1r * Tw_) / (1 - q_1r * deltaIce_) * deltaIce_;
	
	volScalarField Twater = Tf_ + (q_0 + q_1 * Tf_ ) / (1.0 - q_1 * delta_) * delta_;
	
	Info << "MIN/MAX Ts_ " << min(Ts_) << " " << max(Ts_) << endl;
	Info << "MIN/MAX Tw_ " << min(Tw_) << " " << max(Tw_) << endl;
	Info << "MIN/MAX T_ " << min(T_) << " " << max(T_) << endl;
	Info << "MIN/MAX Twater  " << min(Twater) << " " << max(Twater) << endl;
	
	/*
	Info << "MIN/MAX TIce" << min(TIce) << " " << max(TIce) << endl;
	
	volScalarField impSpLf_ = -impSp_ * Lf_;
	volScalarField impSpCpiTf_ = - impSp_ * CpIce_ * Tf_;
	volScalarField impSpCpTParcelsTf_ = - impSp_ * Cp_ * (TParcels_ - Tf_);
	
	Info << "MIN/MAX impSpLf_ " << min(impSpLf_) << " " << max(impSpLf_) << endl;
	
	Info << "MIN/MAX impSpCpiTf_ " << min(impSpCpiTf_) << " " << max(impSpCpiTf_) << endl;
	
	Info << "MIN/MAX impSpCpTParcelsTf_ " << min(impSpCpTParcelsTf_) << " " << max(impSpCpTParcelsTf_) << endl;
	
	Info << "==========================================================" << endl;
    
    Info << "MIN/MAX Ts_ " << min(Ts_) << " " << max(Ts_) << endl;
	
	Info << "hsSp_ " << hsSp_ << endl;
    
    Info << "MIN/MAX q_0r " << min(q_0r) << " " << max(q_0r) << endl;
    
    Info << "MIN/MAX q_1r " << min(q_1r) << " " << max(q_1r) << endl;
	
	Info << "htcs_->h(): " << htcs_->h() << endl;
	
	Info << "MIN/MAX impSp_ " << min(impSp_) << " " << max(impSp_) << endl;
	...
	//Info << "TPrimary_ = " << TPrimary_ << endl;
	
	Info << "MIN/MAX TParcels_" << min(TParcels_) << " " << max(TParcels_) << endl;
    
    //Info << "MIN/MAX (q_0r + q_1r * Ts_) / (1 - q_1r * deltaIce_) * deltaIce_  " << min((q_0r + q_1r * Ts_) / (1 - q_1r * deltaIce_) * deltaIce_  << " " << max((q_0r + q_1r * Ts_) / (1 - q_1r * deltaIce_) * deltaIce_ ) << endl; 
    
    Info << "TIce = " << TIce << endl;
	*/
    
    volScalarField Wh = pos0(pos0(Ts_ - Tf_) + alpha_);
    
    // Info << "Max Wh" << max(Wh) << endl;   
